...
class LabColorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            rgb = imageio.imread(self.files[idx])
            return torch.Tensor(im), JPEGname, torch.Tensor(rgb)
...
def norm_lab_to_rgb(L, ab, norm=True, filt=False, bw=False):
    ...
    rgb = [color.lab2rgb(l.transpose(1, 2, 0)) for l in lab]
    return np.array(rgb)
...
if c.dataset == 'imagenet':
    ...
    # test_dir = '../../../data/cINN/coco17/test'       # 后缀 jpg
    # test_dir = '../../../data/cINN/4738_gray'           # 后缀 PNG
    # test_dir = '../../../data/cINN/sample'            # 后缀 PNG

    test_dir = '/dat01/liuting/data/pic/lightColor-256'
